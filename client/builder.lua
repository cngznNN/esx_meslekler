...
Citizen.CreateThread(function()
	while true do
		...
		if DoesEntityExist(objectWheelBarrow) and IsEntityAnObject(objectWheelBarrow) then
			...
			if IsEntityPlayingAnim(GetPlayerPed(-1), 'amb@lo_res_idles@', 'world_human_muscle_free_weights_lo_res_base', 3) == false then
				Citizen.Wait(2500)
				builderstartAnim('amb@lo_res_idles@', 'world_human_muscle_free_weights_lo_res_base')
			end
		else
        ...
        for _, v in pairs(Config.Builder) do
            ...
            if LoadProduct then
                for i=1, #v.LoadProduct, 1 do
                    ...
                    if Vdist(pedCoords.x, pedCoords.y, pedCoords.z, 
                    v.LoadProduct[i].x, v.LoadProduct[i].y, v.LoadProduct[i].z) < 6.0 then
        ...
        function(data, menu)
            ...
            if data.current.value == 'take' then
                ...
                local coords = {
                    ...
                    z = GetEntityCoords(GetPlayerPed(-1)).z
                }
                local ped = GetPlayerPed(-1)
    ...
    if bool then
        ...
        for _, v in pairs(Config.Builder) do
            ...
            for i=1, #v.UnloadingProduct, 1 do
				...
				SetBlipScale(UnloadingProduct, 1.1)
...
function builderstartAnim(lib, anim)
	...
	Citizen.CreateThread(function()
	  ...
	  TaskPlayAnim(GetPlayerPed(-1), lib ,anim , 4.00, -4.00, -1, 63, 0.00, false, false, false)
	end)
